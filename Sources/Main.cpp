...
#	endif
	...
	if (isServer) {
		...
		if (PreferencesCache::MaxFps == PreferencesCache::UseVsync) {
			...
			if (PreferencesCache::MaxFps > 0) {
				LOGI("Frame limit set to {} FPS", PreferencesCache::MaxFps);
			}
		}
#if !defined(DEATH_TARGET_SWITCH) && !defined(DEATH_TARGET_PSP) && !defined(DEATH_TARGET_VITA) && !defined(DEATH_TARGET_WII) && !defined(DEATH_TARGET_GAMECUBE) && !defined(DEATH_TARGET_3DS) && !defined(DEATH_TARGET_DREAMCAST) && !defined(DEATH_TARGET_N64) && !defined(DEATH_TARGET_AMIGAOS)
	...
	if (_networkManager != nullptr && _networkManager->GetState() != NetworkState::Listening) {
		InvokeAsync([this, reason]() {
			...
			switch (reason) {
				...
				case Reason::ProtocolViolation: mainMenu->SwitchToSection<Menu::SimpleMessageSection>(_("\f[c:#704a4a]Connection has been closed!\f[/c]\n\n\nThe client was rejected due to malformed communication protocol."), true); break;
